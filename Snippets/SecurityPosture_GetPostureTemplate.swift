...
func sample(
  ...
  postureTemplateId: String
) async throws {
  let response = try await client.getPostureTemplate(
    request: GetPostureTemplateRequest()
      .with {
        $0.name =
          "organizations/\(organizationId)/locations/\(locationId)/postureTemplates/\(postureTemplateId)"
      }
  )
